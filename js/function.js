  ...
  $("a[data-href], button[data-href]")
    ...
    .on("click", (e) => {
      ...
      if (href === "login") {
        ...
        new AJAX({ mvc: "user.setLogout" }, (response) => {
          // re-render menu so Login link reappears
          new AJAX({ mvc: "menu.display" }, showMenu);
        });
      } else if (href === "register") {
        new AJAX(
          { mvc: "login.display", template: "modal.register" },
          showModal,
        );
      } else if (href === "getpassword") {
        new AJAX(
          { class: "password.getPassword", password: "" },
          (response) => {
...
function loadedForm(id) {
  $(`#${id}`).on("submit", (e) => {
    ...
    new AJAX(data, (response) => {
      if (response.html) {
        ...
        // refresh menu so login/logout state updates
        new AJAX({ mvc: "menu.display" }, showMenu);
      } else {
        // ??
    ...
    .on("input", (e) => {
      if ($(e.target).val().length == 0) {
        ...
        $(".password + .progress .progress-bar")
          ...
          .removeClass(
            "bg-success bg-warning bg-danger bg-info bg-light bg-dark",
          );
        // end add
      } else {
        new AJAX(
          ...
          (response) => {
            $(e.target)
              .addClass(response.html.code ? "is-invalid" : "is-valid")
              .removeClass(response.html.code ? "is-valid" : "is-invalid")
              .trigger("change");
            // change progress bar
            let bgClass = ((percent) => {
              if (percent >= 1) {
  ...
  $("#form-register input").on("change", (e) => {
    $('#form-register [type="submit"]').prop(
      "disabled",
      $("#form-register input").length !=
        $("#form-register input.is-valid").length,
    );
  });
}
